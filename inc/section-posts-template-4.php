<?php 
	/* 
	** @package makzine
	** Homepage Category Posts Four
	*/

class Makzine_Category_Post_Section_Four extends WP_Widget {
	
	function __construct() {
		
		$options = array(
			'classname' => 'makzine-category-post-section-four',
			'description' => 'Makzine homepage Category Posts type four',
		);

		parent::__construct('makzine_category_post_section_four', 'Makzine Category Posts Four', $options);
	...
	public function form($instance) {
        ...
        $posts_per_page = ( !empty( $instance[ 'posts_per_page' ] ) ? absint($instance[ 'posts_per_page' ] ) : 0  );
	...
	public function widget($args, $instance) {
		...
		$per_page = (($posts_per_page == 1) ? '1' : '2');

		// large thumb

        $cat_post_section_four_lg_args = array (
            'post_type'        => 'post',
            'posts_per_page'   => $per_page,
	        'cat'              => $cat
        );

		$cat_post_section_four_lg_query = new WP_Query($cat_post_section_four_lg_args);

		echo $args['before_widget']; 

		?>
		
				<div class="category-section-four border-btm">
					...
					<div class="section-widget-content">
						...
						<div class="col-sm-12 sc-tw-lg-post">
							...
							// large thumb
								...
								if( $cat_post_section_four_lg_query->have_posts() ): 

									while ( $cat_post_section_four_lg_query->have_posts() ): 

										$cat_post_section_four_lg_query->the_post(); 
							...
							?>
								...
								<article class="cat-post col-sm-6">
									...
									<?php if( has_post_thumbnail() ):
                          	 			...
                          	 			$url = wp_get_attachment_url( get_post_thumbnail_id($cat_post_section_four_lg_query->ID) ); 
				...
				// small thumb
				$total = $posts_per_page - $per_page;
				
		        $cat_post_section_four_sm_args = array (
		            'post_type'        => 'post',
				...
				$cat_post_section_four_sm_query = new WP_Query($cat_post_section_four_sm_args);
				...
				?>
						...
						<div class="col-sm-12 sc-o-sm-post">
							<div class="row">
							...
							// large thumb
								...
								if( $cat_post_section_four_sm_query->have_posts() ): 

									while ( $cat_post_section_four_sm_query->have_posts() ): 

										$cat_post_section_four_sm_query->the_post(); 

							?>

						
								<article class="cat-post col-sm-6 clearfix">

									<?php if( has_post_thumbnail() ):
                          	
                          	 			$url = wp_get_attachment_url( get_post_thumbnail_id($cat_post_section_four_sm_query->ID) ); 
...
add_action('widgets_init', function() {
	register_widget('Makzine_Category_Post_Section_Four');
} );
